orion.dictionary.addDefinition('fireplaces', 'parallax',
  orion.attribute('image', {
      label: 'Fireplaces Image',
      optional: true
  })
);

orion.dictionary.addDefinition('stoves', 'parallax',
  orion.attribute('image', {
      label: 'Stoves Image',
      optional: true
  })
);

orion.dictionary.addDefinition('outdoorlivingproducts', 'parallax',
  orion.attribute('image', {
      label: 'Outdoor Living Image',
      optional: true
  })
);

orion.dictionary.addDefinition('protableandbuiltingrills', 'parallax',
  orion.attribute('image', {
      label: 'Grills Image',
      optional: true
  })
);

orion.dictionary.addDefinition('playsets', 'parallax',
  orion.attribute('image', {
      label: 'Playsets Image',
      optional: true
  })
);

orion.dictionary.addDefinition('shedsandanimalenclosures', 'parallax',
  orion.attribute('image', {
      label: 'Sheds/Animal Enclosures Image',
      optional: true
  })
);

orion.dictionary.addDefinition('libertysafes', 'parallax',
  orion.attribute('image', {
      label: 'Liberty Safes Image',
      optional: true
  })
);

orion.dictionary.addDefinition('furnacesandboilers', 'parallax',
  orion.attribute('image', {
      label: 'Furnaces/Boilers Image',
      optional: true
  })
);


// orion.dictionary.addDefinition('muebles', 'images',
//   orion.attribute('image', {
//       label: 'Imagen categoría muebles',
//       optional: true
//   })
// );

// orion.dictionary.addDefinition('pisos-revestimientos', 'images',
//   orion.attribute('image', {
//       label: 'Imagen categoría pisos & revestimientos',
//       optional: true
//   })
// );

// orion.dictionary.addDefinition('decoracion', 'images',
//   orion.attribute('image', {
//       label: 'Imagen categoría decoración',
//       optional: true
//   })
// );

// orion.dictionary.addDefinition('iluminacion', 'images',
//   orion.attribute('image', {
//       label: 'Imagen categoría iluminación',
//       optional: true
//   })
// );

// orion.dictionary.addDefinition('exterior', 'images',
//   orion.attribute('image', {
//       label: 'Imagen categoría exterior',
//       optional: true
//   })
// );

// orion.dictionary.addDefinition('banos', 'images',
//   orion.attribute('image', {
//       label: 'Imagen categoría baños',
//       optional: true
//   })
// );

// orion.dictionary.addDefinition('cocina', 'images',
//   orion.attribute('image', {
//       label: 'Imagen categoría cocina',
//       optional: true
//   })
// );

// orion.dictionary.addDefinition('infantil', 'images',
//   orion.attribute('image', {
//       label: 'Imagen categoría infantil',
//       optional: true
//   })
// );

// orion.dictionary.addDefinition('decoradores', 'images',
//   orion.attribute('image', {
//       label: 'Imagen decoradores',
//       optional: true
//   })
// );
